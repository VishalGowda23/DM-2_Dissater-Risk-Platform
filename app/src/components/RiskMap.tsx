  ...
  return (
    <div className="h-full w-full relative">
                ...
                <div className="p-2 min-w-[200px]">
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-600">{t('wardId')}</span>
                      <span className="font-mono font-bold">{ward.ward_id}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">{t('populationLabel')}</span>
                      <span className="font-bold">{ward.population?.toLocaleString()}</span>
                    </div>
                    {risk && (
                      <>
                        <div className="flex justify-between">
                          <span className="text-gray-600">{t('topHazardLabel')}</span>
                          <span className="font-bold uppercase">{t(hazardKey(risk.top_hazard))}</span>
                        </div>
                        <div className="flex justify-between items-center mt-2 pt-2 border-t">
                          <span className="text-gray-600">{t('riskScoreLabel')}</span>
                          <span 
                        ...
                        {risk.flood && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-500">{t('floodLabel')}</span>
                            <span className="font-mono">{risk.flood.event?.toFixed(0)}%</span>
                          </div>
                        )}
                        {risk.heat && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-500">{t('heatLabel')}</span>
      ...
      <div className="absolute bottom-4 left-4 bg-white border-2 border-black p-3 shadow-lg z-[1000]">
        <h4 className="font-bold text-sm mb-2 uppercase">{t('riskLevel')}</h4>
